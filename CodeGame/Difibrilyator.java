...
public class Difibrilyator {
	...
	public static void main(String[] args) {
		...
		double d;// дистанция
		for (int i=0;i<N;i++) {
			String[] def=scan.nextLine().replace(',', '.').split(";");// разделяем строку на элементы массива ";"
			//System.out.println(Arrays.toString(def));
			d=distance(lon,lot,Double.parseDouble(def[4]),Double.parseDouble(def[5]));// метод для расчета расстаяния
			if(d<min) {
				min=d;
				answer=def[1];
			}
	...
	private static double distance(double lon, double lot, double lonB, double lotB) {
		double x=(lonB-lon)*Math.cos((lotB+lot)/2);
		double y=lotB-lot;
		return Math.sqrt(y*y+x*x)*6371;
	}
